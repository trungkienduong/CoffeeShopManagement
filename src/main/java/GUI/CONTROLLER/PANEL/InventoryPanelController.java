...
public class InventoryPanelController {
    ...
    private void handleAddInventory(ActionEvent event) {
        if (currentUser != null && currentUser.hasRole("EMPLOYEE")) {
            showAlert(Alert.AlertType.WARNING, "You do not have permission to add inventory.");
            return;
        }
        ...
        try {
            ...
            showAlert(Alert.AlertType.ERROR, "Failed to load the add inventory dialog.");
        }
    }
    ...
    private void handleDeleteInventory(ActionEvent event) {
        if (currentUser != null && currentUser.hasRole("EMPLOYEE")) {
            showAlert(Alert.AlertType.WARNING, "You do not have permission to delete inventory.");
            return;
        }

        Inventory selected = inventoryTable.getSelectionModel().getSelectedItem();
        if (selected == null) {
            showAlert(Alert.AlertType.WARNING, "Please select an inventory item to delete.");
            return;
        }
        ...
        try {
            ...
            if (success) {
                showAlert(Alert.AlertType.INFORMATION, "Inventory item deleted successfully.");
                loadInventoryData();
            } else {
                showAlert(Alert.AlertType.ERROR, "Failed to delete inventory item. It might be in use or does not exist.");
            }
        } catch (Exception e) {
            ...
            if (errorMessage != null) {
                ...
                if (lowerErrorMsg.contains("reference constraint") ||
                        lowerErrorMsg.contains("foreign key") ||
                        lowerErrorMsg.contains("fk__product_r__item") ||
                        lowerErrorMsg.contains("delete statement conflicted")) {

                    showAlert(Alert.AlertType.ERROR,
                            "Cannot delete this item because it is being used in product recipes.\n" +
                                    "Please remove or update those product recipes first.");
                } else {
                    showAlert(Alert.AlertType.ERROR,
